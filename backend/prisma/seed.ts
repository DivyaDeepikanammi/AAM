import { MealType, PrismaClient, ReminderType } from '@prisma/client';
import * as bcrypt from 'bcryptjs';

const prisma = new PrismaClient();

const recipeCatalog: Record<string, string[]> = {
...
function recipeType(name: string) {
...
async function main() {
  ...
  await prisma.meal.createMany({
    data: [
      { date: today(), type: MealType.BREAKFAST, menuItem: 'Idli + Egg', curryName: null, portion: '2 idlis + 1 egg', alternative: 'Dosa + Egg' },
      { date: today(), type: MealType.LUNCH, menuItem: '1 cup rice + Bendakaya Pulusu + carrot beetroot curd salad', curryName: 'Bendakaya Pulusu', portion: '1 cup rice', alternative: 'Bendakaya Fry + Rasam', recipeId: bendakaya?.id },
      { date: today(), type: MealType.DINNER, menuItem: '2 chapatis + Egg Curry', curryName: 'Egg Curry', portion: '2 chapatis', alternative: 'Egg Bhurji + cucumber curd', recipeId: egg?.id },
    ],
  });
  ...
  await prisma.reminder.createMany({
    data: [
      { type: ReminderType.BREAKFAST, title: 'Breakfast', time: '8:00 AM' },
      { type: ReminderType.MORNING_TABLET, title: 'Morning tablet', time: '8:30 AM' },
      { type: ReminderType.LUNCH, title: 'Lunch', time: '1:00 PM' },
      { type: ReminderType.AFTERNOON_TABLET, title: 'Afternoon tablet', time: '2:00 PM' },
      { type: ReminderType.WALKING, title: 'Walking', time: '6:00 PM' },
      { type: ReminderType.DINNER, title: 'Dinner', time: '7:30 PM' },
      { type: ReminderType.NIGHT_TABLET, title: 'Night tablet', time: '8:30 PM' },
      { type: ReminderType.WATER, title: 'Water', time: 'All day' },
      { type: ReminderType.MONTHLY_SUGAR_CHECK, title: 'Monthly sugar check', time: 'Monthly' },
    ],
  });
